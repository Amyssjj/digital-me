...
| `DIGITAL_ME_BRAIN_DB` | no | `~/.openclaw/data/brain.db` (the retired `task-orchestrator.db` is no longer read anywhere) | brain-orchestrator | `services/dream-cycle` brain-learnings + citations readers; `services/digest` handoff reader (`digest.config.resolve_brain_db`) |

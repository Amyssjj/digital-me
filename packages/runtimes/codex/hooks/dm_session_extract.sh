#!/usr/bin/env bash
# Digital Me (Codex): skinny audit log of Codex sessions for protocol
# compliance. One JSONL line appended per Stop. Not ingested by the dream
# cycle. Silent side-effect — never blocks, never emits stdout. Fails open.
...
if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ]; then
  grep -qE '"(name)"[[:space:]]*:[[:space:]]*"(memory_search|mcp__openclaw_brain__memory_search|mcp__openclaw-brain__memory_search)"' "$TRANSCRIPT" 2>/dev/null && MEMORY_SEARCH=1

#!/usr/bin/env bash
# Digital Me (Codex): inject openclaw-brain memory_search hits into user prompts.
# Runs on Codex UserPromptSubmit. Fails open (empty output, exit 0) on any error.
#
# Codex's UserPromptSubmit hook stdin carries `.prompt` and `.session_id` —
# identical to Claude Code's — and we emit the same
# `hookSpecificOutput.additionalContext` shape, so the surfacing logic is a
# straight port. The only runtime-specific bits are the M1 emit flags
# (runtime/platform = codex, codex WAL) and the per-session dedup cache key.
#
# Mirrors the Claude Code inject hook:
#   - SCORE_GATE: drop hits below MIN_SCORE.
...
#   - ACTION_FRAMING: force-name-relevance closing ([Digital Me] prefix).
...
# Per-session dedup cache. `codex` prefix keeps it separate from the Claude
# Code cache so the two runtimes never cross-dedup each other.
SEEN_FILE=""
...
# Both append to ~/.openclaw/data/m1_events_codex.jsonl as the durable WAL;
# brain POSTs are best-effort (m1_backfill.py replays on next reachable

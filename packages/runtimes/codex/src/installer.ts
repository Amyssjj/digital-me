 ...
 * to wire CODEX.md + the digital-me-brain MCP server into ~/.codex/.
 *
...
import { BRAIN_MCP_SERVER_NAME, LEGACY_BRAIN_MCP_SERVER_NAMES } from "@digital-me/contracts";
...
/**
 * Move a pre-rename `[mcp_servers.openclaw-brain]` server (and its sub-tables:
 * `.env`, `.tools.*` per-tool approvals the operator chose) to
 * `[mcp_servers.digital-me-brain]`. Run before {@link mergeMcpServer}, which
 * then replaces the main block as usual — so an upgrade leaves exactly one
 * brain server, with the operator's tool approvals carried over.
 *
 * If a `digital-me-brain` server already exists, the legacy tables are dropped
 * instead of renamed: two tables with the same header are a whole-file TOML
 * error that would take every MCP server in config.toml down.
 */
export function renameLegacyMcpServers(existingToml: string): string {
  const current = `mcp_servers.${BRAIN_MCP_SERVER_NAME}`;
  const belongsTo = (t: string, base: string): boolean =>
    t === `[${base}]` || t.startsWith(`[${base}.`);
  const lines = existingToml.split("\n");
  const hasCurrent = lines.some((l) => belongsTo(l.trim(), current));
  const out: string[] = [];
  let dropping = false;
  for (const line of lines) {
    const t = line.trim();
    if (t.startsWith("[") && t.endsWith("]")) {
      const legacy = LEGACY_BRAIN_MCP_SERVER_NAMES.find((n) => belongsTo(t, `mcp_servers.${n}`));
      dropping = legacy !== undefined && hasCurrent;
      if (legacy !== undefined && !hasCurrent) {
        out.push(line.replace(`mcp_servers.${legacy}`, current));
        continue;
      }
    }
...
/**
 * Build the `[mcp_servers.digital-me-brain]` TOML stanza pointing at the
 ...
 * `[mcp_servers.digital-me-brain]` block in place.
 */

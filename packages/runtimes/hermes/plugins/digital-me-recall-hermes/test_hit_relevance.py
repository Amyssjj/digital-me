"""Unit tests for the Hermes recall plugin's `_hit_score` — the
...
plugin's `_hit_score` takes the larger of `vectorScore` (cosine, 0..1) and
`score`, which falls back to `score` for gateway-shaped hits that carry only
`score` (the gateway's fused `score` is never above its `vectorScore`).
...
def test_prefers_vector_score_over_fused_score():
    ...
    assert dmrh._hit_score(hit) == 0.78
    assert dmrh._hit_score(hit) >= dmrh.MIN_SCORE


def test_falls_back_to_score_for_gateway_shaped_hits():
    hit = {"path": "../digital-me/wiki/x.md", "score": 0.47}
    assert dmrh._hit_score(hit) == 0.47


def test_missing_or_null_scores_count_as_zero():
    assert dmrh._hit_score({"path": "wiki/x.md"}) == 0.0
    assert dmrh._hit_score({"path": "wiki/x.md", "score": None}) == 0.0
    assert dmrh._hit_score({"vectorScore": None, "score": None}) == 0.0


def test_non_numeric_scores_count_as_zero():
    assert dmrh._hit_score({"score": "high"}) == 0.0
    assert dmrh._hit_score({"vectorScore": {"nested": 1}}) == 0.0
...
def test_gate_keeps_brain_host_hit_and_drops_weak_one():
    ...
    kept = [h for h in (strong, weak) if dmrh._hit_score(h) >= dmrh.MIN_SCORE]
    assert kept == [strong]

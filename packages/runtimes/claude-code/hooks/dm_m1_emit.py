...
def _resolve_gateway_url() -> str:
    ...
    dm_memory_search_inject.sh): DIGITAL_ME_BRAIN_URL (brain-host) →
...
def _load_gateway_token() -> Optional[str]:
    """Token precedence, in lock-step with _resolve_gateway_url():
      ...
      1. DIGITAL_ME_BRAIN_TOKEN when DIGITAL_ME_BRAIN_URL is set. Both are
         ...
         NEVER falls back to an openclaw token. (An exported
         OPENCLAW_GATEWAY_TOKEN used to win here via the --token default, so
         events reached the WAL but never brain-host.)
...
def _selftest_token_precedence() -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            # DIGITAL_ME_OPENCLAW_CONFIG is consulted first.

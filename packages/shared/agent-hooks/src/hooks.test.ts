...
describe.skipIf(missingTools.length > 0)(`${RUNTIME} hook scripts against a brain-host stub`, () => {
  ...
  // The inject header names the server; transcripts from before the
  // openclaw-brain → digital-me-brain rename carry the old one and still count.
  it.runIf(RUNTIME === "claude-code").each(["digital-me-brain", "openclaw-brain"])("dm_application_rate.sh resolves dm_m1_emit.py from its own directory and emits to brain-host (%s header)", async (server) => {
    const sid = newSession();
    const transcript = path.join(tmp, "transcript.jsonl");
    const injected = [
      `Digital Me / ${server} memory_search top hits for this prompt (auto-injected; may be stale):`,
      "",
  ...
  it.runIf(RUNTIME === "codex")("dm_application_rate.sh scores the codex session from the inject hook's SEEN_FILE + the rollout transcript and emits to brain-host", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        JSON.stringify({
          type: "response_item",
          payload: { type: "function_call", name: "mcp__digital_me_brain__memory_get", arguments: JSON.stringify({ path: "dashboard/kanban.md" }) },
        }),
...
describe.skipIf(missingTools.length > 0)(`${RUNTIME} PreToolUse / Stop hooks that never call the brain`, () => {
  ...
  describe("dm_handoff_reminder.sh", () => {
    ...
    const ccWork = [
      { type: "assistant", message: { content: [{ type: "tool_use", name: "mcp__digital-me-brain__tasks", input: { action: "run_goal" } }] } },
      { type: "assistant", message: { content: [{ type: "tool_use", name: "Edit", input: { file_path: "/x" } }] } },
    ];
    const codexWork = [
      { type: "response_item", payload: { type: "function_call", name: "mcp__digital_me_brain__tasks", arguments: JSON.stringify({ action: "checkpoint" }) } },
      { type: "response_item", payload: { type: "custom_tool_call", name: "apply_patch", input: "*** Begin Patch" } },
    ];
    // The same sessions recorded before the openclaw-brain → digital-me-brain rename.
    const legacy = (lines: unknown[]): unknown[] =>
      JSON.parse(JSON.stringify(lines).replace(/digital([-_])me\1brain/g, "openclaw$1brain")) as unknown[];
    ...
    it("blocks once when a dispatched task session changed files without a handoff (both transcript formats), naming this runtime's tools", () => {
      for (const lines of [ccWork, codexWork, legacy(ccWork), legacy(codexWork)]) {
        ...
        expect(out.reason).toContain(RUNTIME === "codex" ? "no digital-me-brain tasks action=handoff" : "no mcp__digital-me-brain__tasks action=handoff");
      }
    });

    it("stays silent after a handoff, on the re-entrant stop, and for ad-hoc sessions", () => {
      const handedOff = transcript([...ccWork, { type: "assistant", message: { content: [{ type: "tool_use", name: "mcp__digital-me-brain__tasks", input: { action: "handoff" } }] } }]);
  ...
  describe("dm_session_extract.sh", () => {
    ...
    it(`appends one audit line per Stop to ~/${RUNTIME_HOME}/audit, detecting memory_search in either transcript format`, () => {
      ...
      fs.writeFileSync(cc, JSON.stringify({ type: "assistant", message: { content: [{ type: "tool_use", name: "mcp__digital-me-brain__memory_search", input: { action: "handoff" } }] } }) + "\n");

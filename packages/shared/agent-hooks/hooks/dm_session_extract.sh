...
if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ]; then
  # Claude Code names the tool mcp__digital-me-brain__memory_search; Codex uses
  # the underscore server name or the bare tool name. Pre-rename transcripts
  # say openclaw-brain.
  grep -qE 'mcp__(digital[-_]me|openclaw)[-_]brain__memory_search|"(name|tool_name)"[[:space:]]*:[[:space:]]*"memory_search"' "$TRANSCRIPT" 2>/dev/null && MEMORY_SEARCH=1

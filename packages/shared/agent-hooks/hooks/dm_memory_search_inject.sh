#!/usr/bin/env bash
# Digital Me: inject digital-me-brain memory_search hits into user prompts.
...
# script so the hook still finds brain-host when DIGITAL_ME_BRAIN_URL does not
# reach it (settings.json `env` normally carries it; a host that runs hooks
# without it — as Codex does — falls back to the gateway otherwise). Read only
# when the env has no DIGITAL_ME_BRAIN_URL; each of its two keys fills in only
# what the env leaves unset. Parsed line by line, never sourced: other keys
# (and so `#` comments) are ignored, one pair of surrounding quotes is
...
CTX="Digital Me / digital-me-brain memory_search top hits for this prompt (auto-injected; may be stale — verify against current state before acting):

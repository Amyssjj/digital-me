...
#   - the brain tasks tool: `mcp__openclaw-brain__tasks` (Claude Code),
#     `mcp__openclaw_brain__tasks` or bare `tasks` (Codex).
...
if ! grep -qE 'mcp__openclaw[-_]brain__tasks|"(name|tool_name)"[[:space:]]*:[[:space:]]*"tasks"' "$TRANSCRIPT" 2>/dev/null; then
  exit 0
fi
...
case "$DM_RUNTIME" in
  codex)
    ...
    TASKS_TOOL="openclaw-brain tasks"
    ;;
  *)
    WORK_TOOLS="Edit/Write/NotebookEdit"
    TASKS_TOOL="mcp__openclaw-brain__tasks"
    ;;
esac
